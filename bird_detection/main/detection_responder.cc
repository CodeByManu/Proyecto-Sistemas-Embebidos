...
#define MOTOR_GPIO_PIN GPIO_NUM_5

void setup_gpio() {
  gpio_reset_pin(LED_GPIO_PIN);
  gpio_set_direction(LED_GPIO_PIN, GPIO_MODE_OUTPUT);
  gpio_set_level(LED_GPIO_PIN, 0);
  gpio_reset_pin(MOTOR_GPIO_PIN);
  gpio_set_direction(MOTOR_GPIO_PIN, GPIO_MODE_OUTPUT);
...
void RespondToDetection(float person_score, float no_person_score) {
  ...
  if (person_score_int <= 5) { 
    ...
    bsp_display_unlock();
#endif // DISPLAY_SUPPORT
  MicroPrintf("bird score:%d%%, no bird score %d%%",
